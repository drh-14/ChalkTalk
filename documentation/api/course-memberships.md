...
Adds the authenticated user to a course. Join links and QR codes contain both courseId and joinCode. New members always enter as students.
...
Retrieves a course member. The ETag protects later role changes or removal.
...
##### `UpdateMembershipRequest` (required)
...
Removes the authenticated user from a course.
