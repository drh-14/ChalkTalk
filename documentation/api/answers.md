...
- **`POST /api/v1/posts/{postId}/answers`**
  - Description: Creates an answer to a question post. The server derives kind as student or staff from the caller’s role. Each question has at most one student answer and one staff answer.
...
- **`GET /api/v1/posts/{postId}/answers`**
  - Description: Lists the answers to a question post. The data array contains at most one student answer and one staff answer.
  - Authentication: Either `Cookie: chalktalk_session=<opaque-session>` or `Authorization: Bearer <opaque-token>`.
  - Access: Course member.
  - Request media: None.
  - Request headers:
...
- **`GET /api/v1/answers/{answerId}`**
  - Description: Retrieves an answer.
  - Authentication: Either `Cookie: chalktalk_session=<opaque-session>` or `Authorization: Bearer <opaque-token>`.
  - Access: Course member.
  - Request media: None.
  - Request headers:
...
- **`PATCH /api/v1/answers/{answerId}`**
  ...
  - Query parameters:
    - None.
  - Request body:
    - Schema: `UpdateContentRequest` (required).
    - `bodyMarkdown` (string, optional): Markdown content.
    - `anonymous` (boolean, optional): Whether to hide contributors or author.
    - `removeAttachmentIds` (list of string, optional): Existing attachments to remove.
  - Multipart parts:
    - `metadata` (`UpdateContentRequest`, required, `application/json`): Existing attachments remain unless metadata.removeAttachmentIds lists them; repeated attachments add files up to the five-file limit.
...
- **`DELETE /api/v1/answers/{answerId}`**
  - Description: Deletes an answer. A tombstone is retained when needed to preserve followups.
  - Authentication: Either `Cookie: chalktalk_session=<opaque-session>` or `Authorization: Bearer <opaque-token>`.
...
- **`PUT /api/v1/answers/{answerId}/endorsement`**
  - Description: Endorses an answer. Repeating a successful endorsement is idempotent when If-Match identifies the current endorsed revision. Endorsement makes the answer immutable.
  - Authentication: Either `Cookie: chalktalk_session=<opaque-session>` or `Authorization: Bearer <opaque-token>`.
  - Access: TA or instructor.
  - Request media: None.
  - Request headers:
...
- **`POST /api/v1/answers/{answerId}/followups`**
  - Description: Creates a followup to an answer.
  - Authentication: Either `Cookie: chalktalk_session=<opaque-session>` or `Authorization: Bearer <opaque-token>`.
  - Access: Course member.
  - Request media: `application/json`, `multipart/form-data`
  - Request headers:
...
- **`GET /api/v1/answers/{answerId}/followups`**
  - Description: Lists the direct followups to an answer. Returns direct followups only. Nested followups are addressed through their parent.
  - Authentication: Either `Cookie: chalktalk_session=<opaque-session>` or `Authorization: Bearer <opaque-token>`.
  - Access: Course member.
  - Request media: None.
  - Request headers:
...
- **`POST /api/v1/followups/{followupId}/followups`**
  - Description: Creates a child followup under another followup.
  - Authentication: Either `Cookie: chalktalk_session=<opaque-session>` or `Authorization: Bearer <opaque-token>`.
  - Access: Course member.
  - Request media: `application/json`, `multipart/form-data`
  - Request headers:
...
- **`GET /api/v1/followups/{followupId}/followups`**
  - Description: Lists the direct child followups under a followup. Returns direct children of the specified followup only. Deeper descendants are addressed through their parent followup.
  - Authentication: Either `Cookie: chalktalk_session=<opaque-session>` or `Authorization: Bearer <opaque-token>`.
  - Access: Course member.
  - Request media: None.
  - Request headers:
...
- **`GET /api/v1/followups/{followupId}`**
  - Description: Retrieves a followup.
  - Authentication: Either `Cookie: chalktalk_session=<opaque-session>` or `Authorization: Bearer <opaque-token>`.
  - Access: Course member.
  - Request media: None.
  - Request headers:
...
- **`DELETE /api/v1/followups/{followupId}`**
  - Description: Deletes a followup. A bodyless, authorless tombstone is retained when it has child followups.
  - Authentication: Either `Cookie: chalktalk_session=<opaque-session>` or `Authorization: Bearer <opaque-token>`.
  - Access: Author or staff.
  - Request media: None.
  - Request headers:
